			...
			if($tugas_akhir->status_tugas_akhir==11){
				...
				//Jika sudah mengajukan sidang
					...
					$informasi_penguji = DB::table('tugas_akhir')
							...
							->leftJoin('dosen', 'dosen_penguji_ta.id_dosen', '=', 'dosen.id_dosen')
							->where([['tugas_akhir.id_mahasiswa', '=', $id_mahasiswa]])
							->get()->first();

					$informasi_sidang = DB::table('pengajuan_sidang')
						->get()->first();
					return view("mahasiswa/pengajuan_sidang_ta", array('tugas_akhir' => $tugas_akhir, 'informasi_ta'=> $informasi_ta,'sidang' => $sidang, 'informasi_sidang'=> $informasi_sidang, 'status'=> $status, 'informasi_penguji'=> $informasi_penguji));
				}
			}
